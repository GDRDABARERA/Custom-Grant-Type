...
package org.wso2.sample.identity.oauth2.grant.refreshgrant;
...
/**
 * Grant Type handler for Grant Type e_refresh_token which is used to get a new access token.
 */
public class EssentialRefreshGrantHandler extends AbstractAuthorizationGrantHandler {

    public static final String PREV_ACCESS_TOKEN = "previousAccessToken";
    public static final String REFRESH_TOKEN = "refresh_token";
    private static Log log = LogFactory.getLog(EssentialRefreshGrantHandler.class);

    @Override
    public boolean validateGrant(OAuthTokenReqMessageContext tokReqMsgCtx)
            throws IdentityOAuth2Exception {
    	
    	log.debug("in EssentialRefreshGrantHandler validateGrant method ");
        ...
        for (RequestParameter parameter: tokenReqDTO.getRequestParameters()) {
            if (REFRESH_TOKEN.equals(parameter.getKey())) {
                refreshToken = parameter.getValue()[0];
            }
        ...
        log.debug("in EssentialRefreshGrantHandler validateGrant method 3");
        return true;
    }
    ...
    public OAuth2AccessTokenRespDTO issue(OAuthTokenReqMessageContext tokReqMsgCtx)
    	...
    	log.info("in EssentialRefreshGrantHandler issue method ");
    	
    	log.info("Before issue the token : tokReqMsgCtx.getAuthorizedUser().getUserName()="+tokReqMsgCtx.getAuthorizedUser().getUserName());
    	
    	
    	Map<String, String> map=new HashMap<String, String>();
    	map.put("userName", tokReqMsgCtx.getAuthorizedUser().getUserName());
		try {
			invokeRemoteLoginAgentApi(map);
		} catch (Exception e1) {
			
			e1.printStackTrace();
			log.error("Something went wrong in verify user call ",e1);
			throw new IdentityOAuth2Exception("Something went wrong in verify user call ",e1);
		}
        ...
        for (RequestParameter parameter: oauth2AccessTokenReqDTO.getRequestParameters()) {
            if (REFRESH_TOKEN.equals(parameter.getKey())) {
                oldRefreshToken = parameter.getValue()[0];
            }
            log.debug("key="+parameter.getKey()+" value="+parameter.getValue()[0]);
        }
        ...
        RefreshTokenValidationDataDO refreshTokenValidationDataDO = tokenMgtDAO
                .validateRefreshToken(oauth2AccessTokenReqDTO.getClientId(), oldRefreshToken);
        ...
        log.debug("After issue the token : tokReqMsgCtx.getAuthorizedUser().getUserName()="+tokReqMsgCtx.getAuthorizedUser().getUserName());
        log.debug("Rajeev : token is "+tokenRespDTO.getAccessToken());
	...
	private Boolean invokeRemoteLoginAgentApi(Map<String,String> map) throws IOException,Exception {
		
		log.info("in invokeRemoteLoginAgentApi method");
	    ...
	    log.info("LoginAPI verify to call for user  "+userName);
	    
		entity = new HttpEntity(headers);

		log.debug("invoked verify method");
		
		Boolean loginApiResponse=loginApiResp.getBody();
		
		log.info("LoginAPI verify response status as "+loginApiResp.getStatusCode());
		log.info("invoked verify api with reponse as ....."+loginApiResponse);
	
		if(loginApiResp.getStatusCode()!=HttpStatus.OK)
		{
			log.error("Got status code as "+loginApiResp.getStatusCode());
			throw new UserStoreException("Got status code as "+loginApiResp.getStatusCode());
		}
